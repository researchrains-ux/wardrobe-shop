...
const CATEGORIES = ['tops','outerwear','bottoms','dresses','knitwear','accessories','other'];

export default function AddItemModal({ onClose, onAdded }) {
  const [form, setForm] = useState({ name:'', description:'', price:'', gender:'women', size:'', condition:'new', category:'tops' });
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (!form.name || !form.price || !form.size) { setError('Name, price, and size are required.'); return; }
    ...
    <div className={styles.overlay}>
      <div className={styles.modal}>
        <div className={styles.modalHeader}>
          <h2 className={styles.modalTitle + ' font-serif'}>Add item</h2>
